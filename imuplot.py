...
def calibrate(file, target, T, shift):
    ...
    t = min(max(data[:, -1])-data[0, -1], T)
    num_sample = int(t * rate_imu)
    if shift >= 0:
        xnew = np.linspace(0, t, num_sample)
        data = np.zeros((T * rate_imu, 4))
        data[shift:num_sample, :3] = f(xnew[:num_sample-shift])
        data[:, -1] = np.linspace(0, T, T * rate_imu)
    else:
        shift = -shift
        xnew = np.linspace(0, t, num_sample)
        data = np.zeros((T * 1600, 4))
        data[:num_sample-shift, :3] = f(xnew[shift:])
        data[:, -1] = np.linspace(0, T, T * rate_imu)
