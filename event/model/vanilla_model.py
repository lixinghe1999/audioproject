...
class AVnet(nn.Module):
    def __init__(self):
        ...
        self.fusion_stage = 6
        self.bottleneck = nn.ModuleList([EncoderLayer(self.original_embedding_dim, 2048, 4) for _ in range(12-self.fusion_stage)])
    ...
    def forward(self, audio, image):
        ...
        for i, (blk_a, blk_i) in enumerate(zip(self.audio.v.blocks, self.image.v.blocks)):
            ...
            if i >= self.fusion_stage:
                bottleneck_token = self.bottleneck[i-self.fusion_stage](torch.cat([bottleneck_token, audio, image]))
