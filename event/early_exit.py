...
def profile(model, test_dataset):
    model.load_state_dict(torch.load('4_0.572118710609352.pth'))
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, num_workers=16, batch_size=16, shuffle=False)
    model.eval()
    model.exit = True
    thresholds = [1]
    with torch.no_grad():
