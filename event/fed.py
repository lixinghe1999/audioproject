...
from utils.train import training_step, prepare_model, collate_fn, zero_shot_eval, eval_step, validate_one_model
import torch
import numpy as np
...
def difference_models_norm_2(model_1, model_2):
    ...
    diff_list.extend(
        [torch.sum((tensor_1_classifier[i] - tensor_2_classifier[i]) ** 2) for i in range(len(tensor_1_classifier))])
...
class Fed_server(Process):
    ...
    def __init__(
            self, init_network, config, train_dataset, test_dataset
        ...
        self.server_network.to(self.device)
        ...
        self.server_network.eval()
        ((_, _, text_features), _), _ = self.server_network(text=[
            [test_dataset.class_idx_to_label[class_idx]]
            for class_idx in sorted(test_dataset.class_idx_to_label.keys())
        ], batch_indices=torch.arange(len(test_dataset.class_idx_to_label), dtype=torch.int64, device=self.device))
        self.text_features = text_features.unsqueeze(1).transpose(0, 1)
    ...
    def evaluate_global(self, train_dataset=None, test_dataset=None):
        ...
        train_loss_per_cls, train_acc_per_cls = validate_one_model(
            self.server_network, train_dataset, self.text_features, self.device)

        # evaluate on test set: per-class loss/acc
        test_loss_per_cls, test_acc_per_cls = validate_one_model(
            self.server_network, test_dataset, self.text_features, self.device)
    ...
    def evaluate_global_all(self, train_dataset=None, test_dataset=None):
        ...
        for idx in range(self.num_clients):
            # evaluate on test set: per-class loss/acc
            train_loss_per_cls, train_acc_per_cls = validate_one_model(
                self.networks[idx], train_dataset, self.text_features, self.device)
            # evaluate on test set: per-class loss/acc
            test_loss_per_cls, test_acc_per_cls = validate_one_model(
                self.networks[idx], test_dataset, self.text_features, self.device)
...
class Fed_client(Process):
    ...
    def train_lt(self, idx):
        ...
        if self.feat_aug:
            ...
            cov = np.average(per_cls_cov, axis=0, weights=self.local_num_per_cls[
                idx])  # covariance for feature dimension, shape: e.g., (128, 128)
            ...
            if self.crt:
                ...
                if self.feat_aug:
                    ...
                    transformed_label_list = torch.tensor(
                        [unique_labels.index(i) for i in label_list])  # e.g., size (n, )
                    ...
                    num_per_cls = np.array(
                        [len(np.where(label_list == t)[0]) for t in unique_labels])  # e.g., size (6, )
                    max_num = max(num_per_cls)
                    gen_nums = [np.array([max_num // num_per_cls[i] - 1 for _ in feats_per_cls[i]]) for i in
