...
if __name__ == '__main__':
    ...
    if dataset in ["Speech"]:
        from data.dataloader import Speech_FL

        per_client_data, per_client_label, \
        test_data, test_label, val_data, val_label, \
        cls_per_client, num_per_cls_per_client, train_num_per_cls = Speech_FL(data_root, config)
    ...
    fed = Fed_server(
        ...
        test_data, test_label, state_list, model_list
    )
    ...
    for i in range(num_client):
        client_i_data = [j.clone() for j in per_client_data[i]]
        client_i_label = per_client_label[i]
        p = Fed_client(
            network, criterion, config, client_i_data, client_i_label, training_num_per_cls, test_data, test_label,
            state_list, model_list, idx=i
        )
        p.daemon = True
        p.start()
