    ...
    if args.mode == 0:
        ...
        for path in ['exp2/HOU/']:
            ...
            for i in index:
                ...
                mic1, Zxx1 = micplot.get_wav(path + files[mic1_index], normalize=True)
                time1 = float(files[mic1_index][:-4].split('_')[1])
                p1, l1, r1 = micplot.peaks_mic(Zxx1)
                mic2, Zxx2 = micplot.get_wav(path + files[mic2_index], normalize=True)
        ...
        for i in index:
            ...
            wave_1, Zxx1 = micplot.get_wav(path_noise + file_noise1, normalize=True)
            wave_2, Zxx2 = micplot.get_wav(path_noise + file_noise2, normalize=True)
            wave_3, Zxx3 = micplot.get_wav(path_clean + file_clean1, normalize=False)
            ...
            Zxx_resize[:, :shift] = 0
            Zxx_imu = np.zeros(np.shape(Zxx1), dtype='complex64')
            Zxx_imu[:freq_bin, :] = Zxx3[:freq_bin, :]
            ...
            axs[4].imshow(Zxx_resize * Zxx_share[:freq_bin, :], extent=[0, 5, rate_imu / 2, 0], aspect='auto')
            plt.show()
            # wf = wave.open('test.wav', 'wb')
            # _, audio = signal.istft(Zxx_imu, nperseg=seg_len_mic, noverlap=overlap_mic, fs=rate_mic)
            # wf.setnchannels(1)
            # wf.setsampwidth(2)
