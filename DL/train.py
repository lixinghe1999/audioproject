...
if __name__ == "__main__":
    ...
    if args.mode == 0:
        ...
        lr = 0.0001
        EPOCH = 10
        dataset = NoisyCleanSet('speech100.json', 'background.json', alpha=(1, 0.1, 0.1, 0.1), ratio=1)
        ...
        for target in candidate:
            ...
            model = nn.DataParallel(A2net()).to(device)
            ckpt = torch.load('pretrain/0.0013439175563689787.pth')
            # for f in os.listdir('checkpoint/new_5min'):
            #     if f[-3:] == 'pth' and f[:len(target)] == target:
            #         pth_file = f
            # ckpt = torch.load('checkpoint/new_5min/' + pth_file)
            # print(pth_file)
        ...
        for target in ['he', 'hou']:
            ...
            model = nn.DataParallel(A2net()).to(device)
            model.load_state_dict(ckpt)
            PESQ, SNR, WER = vibvoice(model, target, 3)
            mean_PESQ = np.mean(PESQ)
            mean_WER = np.mean(WER)
