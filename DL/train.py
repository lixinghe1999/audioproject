...
if __name__ == "__main__":
    ...
    if args.mode == 0:
        ...
        lr = 0.001
        EPOCH = 30
        dataset = NoisyCleanSet('json/speech100.json', 'json/all_noise.json', norm=[[0.0037, 0.005], [0.0011, 0.0036], [0.00075, 0.0030]], ratio=1)
        ...
        for target in candidate:
            ...
            dataset = IMUSPEECHSet('json/noise_imuexp7.json', 'json/noise_gtexp7.json', 'json/noise_wavexp7.json', person=[target], simulate=False, phase=True, norm=paras[target])
        ...
        #change noise type
        for target in source:
            datasets.append(IMUSPEECHSet('json/clean_train_imuexp7.json', 'json/clean_train_wavexp7.json', 'json/background.json', person=[target], minmax=norm_clean[target]))
        train_dataset = Data.ConcatDataset(datasets)
        ...
        PESQ, SNR, LSD = offline_evaluation(model, test_dataset)
