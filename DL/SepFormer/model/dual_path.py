...
class SepformerWrapper(nn.Module):
    ...
    def load_all(self, list):
        self.encoder.load_state_dict(list[0])
        self.decoder.load_state_dict(list[1])
        self.masknet.load_state_dict(list[2])
...
if __name__ == "__main__":
    # encoder = torch.load('CKPT/encoder.ckpt')
    # decoder = torch.load('CKPT/decoder.ckpt')
    # masknet = torch.load('CKPT/masknet.ckpt')
    # model = SepformerWrapper( encoder_kernel_size=16,
    #     encoder_in_nchannels=1,
    #     encoder_out_nchannels=256,
    #     masknet_chunksize=250,
    #     masknet_numlayers=2,
    ...
    #     intra_norm_before=True,
    #     inter_norm_before=True)
    # model.load_all([encoder, decoder, masknet])
    # torch.save(model.state_dict(), 'pretrain.pth')
    x = torch.rand(1, 30000).to('cuda')
    model = SepformerWrapper().to('cuda')
