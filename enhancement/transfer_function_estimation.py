    ...
    if args.mode == 0:
        ...
        for i in range(len(candidate)):
            ...
            for index in range(N):
                response = np.zeros((2, freq_bin_high))
                wave, Zxx, phase = load_stereo(path + files_mic1[index], T, seg_len_mic, overlap_mic, rate_mic, normalize=True)
                data1, imu1 = read_data(path + files_imu1[index], seg_len_imu, overlap_imu, rate_imu)
        ...
        for name in ['he', 'yan', 'hou', 'shi', 'shuai', 'wu', 'liang', "1", "2", "3", "4", "5", "6", "7", "8"]:
            ...
            for index in range(N):
                ...
                wave, Zxx, phase = load_stereo(path + files_mic1[i], T, seg_len_mic, overlap_mic, rate_mic,normalize=True)
                data1, imu1 = read_data(path + files_imu1[i], seg_len_imu, overlap_imu, rate_imu)
                data2, imu2 = read_data(path + files_imu2[i], seg_len_imu, overlap_imu, rate_imu)
                for j in range(int((T - segment) / stride) + 1):
                    clip1 = imu1[:, j * time_stride:j * time_stride + time_bin]
        ...
        for path in ['exp4/he/none/', 'exp4/hou/none/', 'exp4/wu/none/', 'exp4/shi/none/', 'exp6/he/none/', 'exp6/hou/none/',
            ...
            for index in range(N):
                i = index
                imu1, Zxx_imu1 = read_data(path + files_imu1[i], seg_len_imu, overlap_imu, rate_imu)
                imu2, Zxx_imu2 = read_data(path + files_imu2[i], seg_len_imu, overlap_imu, rate_imu)
